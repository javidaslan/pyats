...
class test_loopback(aetest.Testcase):
    ...
    @aetest.setup
    def configure_loopback(self, uut):
        # for every device in uut
        for uu in uut:
            if uu.name.startswith('R'):
                uu.configure('interface loopback 0')
                uu.configure('ip address 10.2.1.1 255.255.255.255')
    
    @aetest.test
    def test_interface(self, uut):
        # for every device in uut
        for uu in uut:
            if uu.name.startswith('R'):
                self.output = uu.execute('sh ip int brie | i Lo').splitlines()
                print(self.output)
